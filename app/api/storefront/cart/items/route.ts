...
import { addCartItem, getCart, removeCartItem, updateCartItem } from '@/lib/provider';
...
async function resolveCartId() {
  ...
  if (!cart) throw new Error('Unable to initialise cart');
  return cart.id;
}
...
export async function POST(request: Request) {
  try {
    ...
    if (cart?.id) response.cookies.set('cart_id', cart.id, sessionCookieOptions());
    return response;
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    const body = await request.json();
    const cartId = await resolveCartId();
    const cart = await updateCartItem(cartId, body.lineItemId, { quantity: body.quantity });
